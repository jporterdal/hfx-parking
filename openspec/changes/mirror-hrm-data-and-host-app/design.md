...
**Accuracy is owed by what is served, not by what is being deleted.** `web/template.html` and the committed `out/` files it generates are on their way out of the repository under this change; neither is reachable from the served application, and the live host reads neither. Their figures, claims and defects are therefore **out of scope to correct**. Where a task names a stale figure or a false sentence — 6.3's browser-storage fallback, 7.3's "Regenerated from the live service", 8.1's hand-written figures — it is satisfied by fixing the served page and the documents a reader is pointed at, and is not made incomplete by an identical string surviving in a deprecated file. Correcting those copies would be work spent on markup this change removes. What must be true is that every live element — `web/app/index.html`, what it fetches, and what links to it — makes no claim the mirror does not support.
...
**The existing page, fed by a JSON API.** `web/template.html` already carries the lists, the map, the triage controls and light and dark rendering in plain JavaScript. Its embedded data is replaced with fetches from the server. There is no build step, which task 5.1 requires, and no working interface is discarded.

**How 5.1 actually did it, and the decision that is left open.** This bullet was written expecting `web/template.html` itself to be converted in place. 5.1 copied it instead: `web/app/index.html` is a fork whose only structural change is `const MAP = __MAP__; const DATA = __DATA__;` becoming two `fetch()` calls, with the rest of the script carried over unmodified. The reason was 4.9 — `src/hotspots.py`'s `write_board()` fills the template to produce the live-baseline board that the mirror-backed derivation is reconciled against, and converting the template in place would have destroyed that baseline while the reconciliation still depended on it. That reason has since expired: 4.9 is complete. So the repository now holds two copies of the same page, and **whether `web/template.html` is folded back into the served page or stays a separate file is a decision this change does not make.** It needs one. Folding it in removes a second place for the same markup to drift; keeping it separate preserves the standalone generated board as an artefact independent of the server. Whichever way it goes, it governs 8.7, which retires `out/` generation and therefore decides whether the template survives at all. It does not govern 6.3 or 7.3: those change the served page only, and per M5 the template's copies of the same two defects are left uncorrected deliberately, because the file is being removed rather than maintained.

**Resolved by the user during orchestration (8.7 released):** `web/template.html` is retired rather than folded back into the served page. `web/app/index.html` already carries its functionality, so once 8.7 confirms nothing the template does is missing from the served page, the file can go. Retirement is conditional on that check: if a gap turns up, it is reported and fixed in the served page first, not papered over by keeping the template. If the file is deleted, the docs must name the last git commit that still contains it, so the full file can be read later with `git show <commit>:web/template.html`.
